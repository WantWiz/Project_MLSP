...
class PrototypeNet(L.LightningModule):
    ...
    def get_similarities(self, x):
      """
      Calculate similarities between input features and prototypes.
      ...
      """
      # Ensure x is flattened if time summarization is used
      if self.time_summarization == "none":
          ...
          raise ValueError(f"Time summarization {self.time_summarization} not supported")

      # Calculate similarities based on the proto_loss method
